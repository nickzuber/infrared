...
let assign_types ~(file : string) ~(program : program) : program * environment =
...
let assign_types_with_debugging ~(file : string) ~(program : program) : program * environment =
  let env : environment = Hashtbl.create 53 in
  Printf.printf "%s\n%s\n"
...
let assign_types ~(file : string) ~(program : program) =
  let (typed_program, env) = if !Settings.debug_mode then
      assign_types_with_debugging ~file ~program
    else
      assign_types ~file ~program
  in
  if !Settings.show_types_in_console then
    TypePrinter.pp_types_within_file file typed_program env;
  typed_program
