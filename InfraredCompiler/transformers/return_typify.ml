...
let rec return_typify_statement (statement : statement) (env : environment) : statement =
  match statement with
...
let transform  (env : environment) (program : program) : program =
  ...
  | TypedInfraredProgram (statements, _env) ->
    let statements' = List.map (fun statement -> return_typify_statement statement env) statements in
